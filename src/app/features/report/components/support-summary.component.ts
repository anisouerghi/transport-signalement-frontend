    ...
    @if (support) {
      <section class="panel p-3 p-md-4 mb-3" aria-labelledby="support-title">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-2">
          <h2 id="support-title" class="h5 mb-0">{{ 'support.title' | translate }}</h2>
          <span class="support-chip">
            <i class="bi bi-bus-front" aria-hidden="true"></i>
            {{ support.supportTypeLabel || support.supportTypeCode || ('support.fallbackType' | translate) }}
          </span>
        </div>
        <div class="gold-rule mb-3"></div>
        <dl class="row mb-0 gy-2">
          <dt class="col-4 text-secondary">{{ 'common.reference' | translate }}</dt>
          <dd class="col-8 mb-0 fw-semibold">{{ support.reference }}</dd>
          <dt class="col-4 text-secondary">{{ 'common.label' | translate }}</dt>
